    ...
    return {
        getStoreTypeList: async (obj, { input }, {token}, info) => 
        {
            const callback = async () => {
                try {
                    var data = await db.sequelize.query("select st.Id, st.Name from StoreTypes st Where st.Name!='Otro' and st.Name!='NA' Order by st.Name", { type: db.sequelize.QueryTypes.SELECT});
