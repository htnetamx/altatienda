...
// validamos deploy de esta linea
const corsOptions = {
  origin: [
    'http://localhost:3000',
    'http://localhost:3001/',
    'http://localhost:3001',
    'https://develop.d2o0zw5vdstdg4.amplifyapp.com/',
    'https://develop.d2o0zw5vdstdg4.amplifyapp.com',
    'https://newstore.netamx.app',
  ],
  credentials: true,
};
...
const context = async ({ req, res }) => {
  try {
    ...
    const { hasToken, token } = await headerCookie.getAuthCookie(
      headers.cookie,
      DEV
    );
    if (!token) {
...
var serverLocal = app.listen(process.env.PORT, () =>
  console.log(
    `El servidor está corriendo en http://localhost:5001${server.graphqlPath}`
  )
);
